...
struct RemoteMediaWebhookClientTests {
    ...
    @Test func gatewayFailureDoesNotReplayACommand() async {
    ...
    @Test func aRouteThatProvesNonDeliveryFallsBackForACommand() async throws {
        let recorder = Recorder(statuses: [404, 200])
        try await RemoteMediaWebhookClient(perform: recorder.perform).send(
            .play,
            selection: selection,
            context: context(urls: ["https://cloud.example/h", "https://external.example/h"])
        )
    ...
    @Test func anAmbiguousNetworkFailureDoesNotReplayACommand() async {
        ...
        await #expect(throws: URLError.self) {
            try await RemoteMediaWebhookClient(perform: recorder.perform).send(
                .next,
                selection: selection,
                context: context(urls: ["https://a.example/h", "https://b.example/h"])
            )
        }
        #expect(recorder.requests.count == 1)
    }

    @Test func anInvalidPersistedEntityCannotReachTheTemplateEndpoint() async {
        let recorder = Recorder(statuses: [200])
        let invalid = RemoteMediaSelection(serverId: "home", entityId: "media_player.bad' }}")
        let invalidContext = RemoteMediaTransportContext(
            selection: invalid,
            lifetime: lifetime,
            webhookURLs: [URL(string: "https://example.com/h")!],
            secret: nil
        )
        await #expect(throws: RemoteMediaError.invalidSelection) {
            try await RemoteMediaWebhookClient(perform: recorder.perform).readState(
                selection: invalid,
    ...
    @Test func everyIdempotentCandidateFailingSurfacesTheLastError() async {
