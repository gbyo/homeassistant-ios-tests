...
struct RemoteMediaSnapshotMapperTests {
    ...
    @Test func completeSong() throws {
        ...
        // The signed `entity_picture` path stays beside the snapshot, never inside it.
        #expect(mapped.artworkSource == "/api/media_player_proxy/media_player.speaker")
        // Home Assistant offered a cover, so its absence here means "not prepared yet".
        #expect(snapshot.artwork == nil)
        #expect(snapshot.artworkDisposition == .deferred)
    ...
    func stateAndMissingMetadata(state: String) throws {
        ...
        // Home Assistant offered no cover at all, which is not the same as one still coming.
        #expect(mapped.artworkSource == nil)
        #expect(snapshot.artwork == nil)
        #expect(snapshot.artworkDisposition == .absent)
    ...
    /// An `entity_picture` that is present but useless says the same thing as none at all.
    @Test func anEmptyArtworkSourceIsNoArtwork() throws {
        let mapped = try mapped(attributes: ["media_title": "Track", "entity_picture": ""])
        #expect(mapped.artworkSource == nil)
        #expect(mapped.snapshot.artworkDisposition == .absent)
    }
    ...
    @Test(arguments: [
        "2026-09-06 12:00:00+00:00",
        "2026-09-06 12:00:00.123456+00:00",
    ])
    func spaceSeparatedTimestamps(_ timestamp: String) throws {
        let snapshot = try mappedSnapshot(attributes: [
            "media_position": 1.0, "media_position_updated_at": timestamp,
        ])
        #expect(snapshot.positionUpdatedAtUnix != nil)
    }
