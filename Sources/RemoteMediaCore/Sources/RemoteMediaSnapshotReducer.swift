...
public enum RemoteMediaSnapshotReducer {
    ...
    public static func reduce(
        ...
        if incoming.hasMeaningfulMedia {
            ...
            // Same track: let the dynamic values move and leave stable metadata alone. Artwork
            // can arrive after the metadata, so a report that is still waiting for it keeps what
            // is already on screen rather than blanking the cover.
            switch incoming.artworkDisposition {
            case .deferred:
                return previous.artwork.map(incoming.withArtwork) ?? incoming
