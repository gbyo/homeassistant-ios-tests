...
///
/// A followed id reaches two very different places: a JSON value, where anything is safe, and a
/// Jinja template the Home Assistant server executes. The template is the reason this is a
/// character-set check and not just a domain prefix: an id carrying an apostrophe closes the quoted
/// literal it sits in, and everything after it is template source the server will run. Ids arrive
/// from outside the app and are persisted, so the check belongs at the boundary rather than at
/// each use, where it could only be a second guess at what is already stored.
///
