...
///
/// The single mapping, whichever way the attributes arrived. Two independent mappings of the same
/// attributes is how two views of the same player drift apart, so callers adapt their input to this
/// rather than reimplementing it.
///
/// This is also the one place a Home Assistant timestamp becomes the wire representation: an
/// ISO-8601 `media_position_updated_at` is parsed here and stored as Unix seconds.
public enum RemoteMediaSnapshotMapper {
    public static func map(
        ...
        // Whether Home Assistant offered artwork at all is decided here and nowhere else. Preparing
        // it is someone else's job, so the descriptor is still nil either way; what this records is
        // the difference between "a cover exists and is not ready yet" and "there is no cover".
        let artworkSource = nonEmpty(attributes["entity_picture"])
        let snapshot = RemoteMediaSnapshot(
            ...
            artwork: nil,
            artworkDisposition: artworkSource == nil ? .absent : .deferred,
        ...
        return .init(snapshot: snapshot, artworkSource: artworkSource)
    }
