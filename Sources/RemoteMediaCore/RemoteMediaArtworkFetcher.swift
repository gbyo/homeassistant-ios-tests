...
public enum RemoteMediaArtworkFetcher {
    ...
    /// Whether this URL is syntactically eligible. The async fetch validates every redirect too.
    ...
    public static func fetch(from url: URL) async -> Outcome {
        await fetch(from: url, configuration: sessionConfiguration(), validator: .init())
    }
        ...
        do {
            ...
            if response.expectedContentLength > maximumBytes {
                ...
                return .init(
                    ...
                    byteCount: Int(response.expectedContentLength), reason: "too large"
                )
            }
            ...
            case let .tooLarge(byteCount):
                session.invalidateAndCancel()
                return .init(
                    data: nil, status: 200, mimeType: mime, byteCount: byteCount,
                    reason: "too large"
                )
            }
        } catch {
            session.invalidateAndCancel()
            return .init(
                data: nil, status: nil, mimeType: nil, byteCount: 0,
    ...
    /// byte at a time, but appending one byte at a time to `Data` is the expensive half.
